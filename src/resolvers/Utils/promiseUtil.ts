...
export async function DeployImageBy3Version(imageUrl: string): Promise<string> {
  ...
  try {
    imageUrl = imageUrl.replace(
      `https://fashiondogam-images.s3.ap-northeast-2.amazonaws.com/${folderName}_temp/`,
      ""
    );
    //Download Image
    ...
    return (
      `https://fashiondogam-images.s3.ap-northeast-2.amazonaws.com/${folderName}/` +
      imageUrl
    );
  } catch (e) {
...
export function ExtractFieldFromList(
  list: any,
  fieldName: string,
  depth: number = 1
): any {
